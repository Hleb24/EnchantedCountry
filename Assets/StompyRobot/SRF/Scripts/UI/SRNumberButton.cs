...
namespace SRF.UI
{
    ...
    public class SRNumberButton : UnityEngine.UI.Button, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private void Apply() {
            double currentValue;
            if (double.TryParse(TargetField.text,NumberStyles.Number, CultureInfo.InvariantCulture,out double parseCurrentValue)) {
                currentValue = parseCurrentValue;
            } else {
                var temp = String.Format(TargetField.text, NumberStyles.Number, CultureInfo.InvariantCulture);
            ...
            var endValue = String.Format(currentValue.ToString(CultureInfo.InvariantCulture), NumberStyles.Number);
            TargetField.text = endValue;
