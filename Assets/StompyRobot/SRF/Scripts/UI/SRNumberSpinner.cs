...
namespace SRF.UI
{
    ...
    public class SRNumberSpinner : InputField
    {
        ...
        public override void OnBeginDrag(PointerEventData eventData)
        {
            ...
            eventData.Use();
            if (double.TryParse(text,NumberStyles.Number, CultureInfo.InvariantCulture,out double parseCurrentValue)) {
                _dragStartAmount = parseCurrentValue;
            } else {
                var temp = String.Format(text, NumberStyles.Number, CultureInfo.InvariantCulture);
                _dragStartAmount = Convert.ToDouble(temp);
            }
        ...
        public override void OnDrag(PointerEventData eventData)
        {
            ...
            if (contentType == ContentType.IntegerNumber) {
                int roundValue = (int)Math.Round(_currentValue);
                var endValue = String.Format(roundValue.ToString(CultureInfo.InvariantCulture), NumberStyles.Number);
                text = endValue;
            }
            else
            {
                var endValue = String.Format(_currentValue.ToString(CultureInfo.InvariantCulture), NumberStyles.Number);
                text = endValue;
            }
        }
