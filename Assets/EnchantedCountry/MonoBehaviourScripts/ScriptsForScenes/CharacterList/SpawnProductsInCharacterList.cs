...
namespace Core.EnchantedCountry.MonoBehaviourScripts.ScriptsForScenes.CharacterList {
	...
	public class SpawnProductsInCharacterList : MonoBehaviour {
		...
		private void SetListsSoAndSpawnProducts() {
			SetArmorListSo(_equipmentsOfCharacterDataHandler.EquipmentsOfCharacter.EquipmentCards, _storage.armorList);
			SetWeaponListSo(_equipmentsOfCharacterDataHandler.EquipmentsOfCharacter.EquipmentCards, _storage.weaponList);
			SetProjectilesListSo(_equipmentsOfCharacterDataHandler.EquipmentsOfCharacter.EquipmentCards, _storage.projectiliesList);
			SetItemListSo(_equipmentsOfCharacterDataHandler.EquipmentsOfCharacter.EquipmentCards, _storage.itemList);
			Spawn();
		}
		...
		private void SetProjectilesListSo(List<EquipmentCard> equipmentCards, List<ProductSO> projectiliesList) {
			...
			for (int i = 0; i < equipmentCards.Count; i++) {
				for (int j = 0; j < projectiliesList.Count; j++) {
		...
		private void SpawnArmor() {
			...
			for (int i = 0; i < _armorListSo.Count; i++) {
				...
				ProductsInCharacterListView productView = product.GetComponent<ProductsInCharacterListView>();
